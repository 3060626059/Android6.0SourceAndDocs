...
public final class ActivityThread {
    ...
    private class ApplicationThread extends ApplicationThreadNative {
        ...
        //参数token对应AMS中这个Service的ServiceRecord
        //serviceInfo描述的是这个service的信息，对应AndroidManifest文件里面的service
        public final void scheduleCreateService(IBinder token,
                ServiceInfo info, CompatibilityInfo compatInfo, int processState) {
            updateProcessState(processState, false);
            CreateServiceData s = new CreateServiceData();
            s.token = token;//对应AMS中的ServiceRecord
            s.info = info;//对应ServiceInfo
            s.compatInfo = compatInfo; //兼容性信息，暂时不考虑
            //注意这个地方并没有设置intent字段

            sendMessage(H.CREATE_SERVICE, s);
        }
        //参数token对应AMS中这个Service的ServiceRecord
        //参数intent其实表示的是一类intent，这类intent构造的Intent.FilterComparison的equals返回true
    ...
    private void handleCreateService(CreateServiceData data) {
        ...
        try {
            //其实这个ClassLoader是PathClassLoader
            //反射创建Service
            java.lang.ClassLoader cl = packageInfo.getClassLoader();
        ...
        try {
            ...
            try {
                //通知服务端，Service启动完成，，，这个地方怎么判断这个函数是不是异步调用的？？？？？？
            ...
            try {
                ...
                try {
                    if (!data.rebind) {
                        ...
                        //第一个参数表示ServiceRecord
                        //参数binder用来返回给发起绑定请求App的，这样他们可以使用这个binder来访问这个Service的相关功能了，
    ...
    private void handleUnbindService(BindServiceData data) {
        //被告知谁不和我绑定了，，因为我可能被多个App进程绑定，所以这个地方可能会执行多次，
        Service s = mServices.get(data.token);
        if (s != null) {
            try {
                data.intent.setExtrasClassLoader(s.getClassLoader());
    ...
    public static void main(String[] args) {
        ...
        //创建了Looper之后再new ActivityThread对象的，，因为这个对象里面创建了Handler
        ActivityThread thread = new ActivityThread();
        thread.attach(false);
