...
public final class SQLiteSession {
    ...
    /**延迟型事务，如果第一条操作是读，那么获取到的是共享锁，否则获取到的是“保留锁”
    ...
    /**立即模式，获取的是“保留锁”，当前SQLiteSession可以读写，其他SQLiteSession只能读
    ...
    /**排斥型事务，获取排他锁，当前SQLiteSession可以读写，其他SQLiteSession不能访问数据库，
    ...
    private void endTransactionUnchecked(CancellationSignal cancellationSignal, boolean yielding) {
        ...
        if (mTransactionStack != null) {
            ...
            //多个事务嵌套的时候，只有当最外层的事务结束的时候，才会执行提交或者回滚操作。
            //只要有一个事务失败，那么这批事务都会失败，进行回滚操作，
            //回调函数commit或者rollback在真正的数据库操作之前发生
            try {
                if (successful) {
    ...
    public long executeForLong(String sql, Object[] bindArgs, int connectionFlags,
        ...
        try {
            ...
            //这个地方可以看到上面用完了连接，马上又释放链接，让链接回到链接池
            releaseConnection(); // might throw
        }
    }
    ...
    private void acquireConnection(String sql, int connectionFlags,
            CancellationSignal cancellationSignal) {
        //必要的时候才获取连接，SQLiteSession和SQLiteConnection是动态绑定的
        if (mConnection == null) {
