...
public final class SQLiteConnection implements CancellationSignal.OnCancelListener {
    ...
    private void dispose(boolean finalized) {
        ...
        if (mConnectionPtr != 0) {
            ...
            try {
                ...
                //关闭链接资源，，，这个地方可以看到调用close或者finalize方法都会关闭这个链接
    ...
    public long executeForLong(String sql, Object[] bindArgs,
        ...
        try {
            ...
            try {
                ...
                //循环到native层绑定参数
                bindArguments(statement, bindArgs);
                //给出读写数据库提示，比如严苛模式下一般不应该读写数据库，那么会触发相应的报警信息
                applyBlockGuardPolicy(statement);
                attachCancellationSignal(cancellationSignal);
                try {
                    //真正的在native层执行语句
    ...
    private PreparedStatement acquirePreparedStatement(String sql) {
        ...
        // 可以看到这个地方返回了一个sql语句在native层的指针值
    ...
    //绑定参数，，从for循环里面也可以看到，PreparedStatement在native层也有相应的资源，这个资源用mStatementPtr来标识
    //
    ...
    //如果语句禁止执行的话，那么抛出异常，，一般是只读的链接里面想做写数据库操作
