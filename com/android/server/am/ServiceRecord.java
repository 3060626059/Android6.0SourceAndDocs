...
final class ServiceRecord extends Binder {
    ...
    //intent到intentBindRecord的映射，依据的是hashcode和equsals方法，，
    //一些特定字段相同的intent就会共用一个IntentBindRecord，尽管这些intent可能来自不同的进程，
    final ArrayMap<Intent.FilterComparison, IntentBindRecord> bindings
            = new ArrayMap<Intent.FilterComparison, IntentBindRecord>();
                            // All active bindings to the service.
    //因为App端可能用同一个ServiceConnection可以多次绑定同一个Service，
    //这个IBinder是APP端的，ServiceConnection
    //当然不同Activity对应不同ServiceConnection绑定同一个Service也是可以的，，
    ...
    public AppBindRecord retrieveAppBindingLocked(Intent intent,
        ...
        //IntentBindRecord和特定的intent对应，，，
        IntentBindRecord i = bindings.get(filter);
        if (i == null) {
            i = new IntentBindRecord(this, filter);
            bindings.put(filter, i);
        }
        //AppBindRecord ，，，那么现在这个AppBindRecord代表特定intent对应的某个进程的AppBindRecord
        //换句话说，同一个进程，针对不同的intent，完全有可能有多个AppBindRecord，，，
