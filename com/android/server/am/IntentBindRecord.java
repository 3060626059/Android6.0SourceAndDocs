...
/**绑定特定service，特定字段相同的intent，      共用的一个数据结构，
 * 换句话说如果这个Service可以有多种intent启动(比如配置了多个intent-filter),那么针对这个Service,会有多个IntentBindRecord,
...
final class IntentBindRecord {
    ...
    IBinder binder;//从Service的onBind方法返回来的值，
    /** Set when we have initiated a request for this binder. */
    //是否已经向Service请求过IBinder(发起绑定请求，，因为绑定Service需要通过AMS进行中转，)
    boolean requested;
    /** Set when we have received the requested binder. */
    //标记已经收到真正的Service的onBind返回的IBinder
    boolean received;
